...
class ETLPipelineSummary:
    ...
    def transform(self, df):
        ...
        new_df1 = self.compute_false_dataset_with_same_room_type_and_price_and_minimum_nights_and_latitude(df)
        ...
        new_df4 = self.compute_false_dataset_with_same_room_type_and_minimum_nights_and_longitude(df)
    ...
    def compute_ground_truth(self, df):
        ...
        new_df = pd.merge(df, df, left_on=['host_id','longitude','latitude'],
                                  right_on=['host_id','longitude','latitude'],
    ...
    def run(self):
        self.extract_csv()
        self.prepare_ground_truth_from_csv()
        df = self.load_csv()
        new_df = self.transform(df)
        self.split_and_save(new_df)
        self.print_statistics()
